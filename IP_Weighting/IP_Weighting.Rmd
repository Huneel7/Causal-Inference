...
## Comparing the result with coefficient of estimate ($Beta_{1}$) from a logistic model
```{r}
...
$N(Y=1 \mid A = 0, L=0) \times W^{A}_{1} = 73 \times 1.37 = 100$

$N(Y=1 \mid A = 1, L=0) \times W^{A}_{1} = 54 \times 3.67 = 198$

$N(Y=1 \mid A = 0, L=1) \times W^{A}_{1} =71 \times 2.77 = 197$

$N(Y=1 \mid A = 1, L=1) \times W^{A}_{1} = 174 \times 1.56 = 272$
...
$LOR_{A=1 \ vs \ A=0} = log\left(\frac{(0.47/0.53)}{(0.297/0.703)}\right) = 0.74$


```{r, echo = FALSE, warning = FALSE}
exposureModel <- glm(A ~ L, data = df, family = "binomial")

df[, pA := predict(exposureModel, type = "response")]

df2 <- defDataAdd(varname = "pA_actual", 
                    formula = "(A * pA) + ((1 - A) * (1 - pA))", 
                    dist = "nonrandom")
df2 <- defDataAdd(df2, varname = "IPW", 
                    formula = "1/pA_actual", 
                    dist = "nonrandom")

df3 <- addColumns(df2, df)
```
...
## Computing the coefficient of $Beta_{1}$ of the marginal structural model
```{r}
msm.w <- geeglm(
  wt82_71 ~ qsmk,
  data = nhefs_uncensored,
...
Here, the parameter estimate, $Beta_{1}$, is 3.4. This estimated value implies that quitting smoking $(A=1)$ increases weight $(Y)$ by 3.4kg on average.  
