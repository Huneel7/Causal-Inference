...
**Scenario 1: Propensity score model is correct** 
...
**Scenario 2: Regression model is correct**
...
$= \ E\left(\frac{A-E(A \mid L)}{E(A \mid L)}E\left[\{Y^{1}-E(Y \mid A=1, L)\}\mid A, L\right] \right)$

$= \ E\left(\frac{A-E(A \mid L)}{E(A \mid L)}\{E[Y^{1} \mid A, L]-E(Y \mid A=1, L)\} \right)$

$= \ E\left(\frac{A-E(A \mid L)}{E(A \mid L)}\{E[Y^{1} \mid A, L]-E(Y^1 \mid A=1, L)\} \right)$

$= \ E\left(\frac{A-E(A \mid L)}{E(A \mid L)}\{E[Y^{1} \mid L]-E(Y^1 \mid L)\} \right) \ \ $ (under no unmeasured confounders)
