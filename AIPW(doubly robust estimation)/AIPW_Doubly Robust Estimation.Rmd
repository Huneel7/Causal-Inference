...
$= \ E\left(\frac{Y^{1}}{P(A=1 \mid L)}E\left[A \mid L \right]\right) \ \ \ \ \ $ (by no unmeasured confounders)

$= \ E\left(\frac{Y^{1}}{E(A=1 \mid L)}E\left[A \mid L \right]\right)$
...
$\therefore \ \hat{\triangle}_{IPW}$ estimates $\triangle$ when the propensity model is identical to the true propensity score.
$\\$
